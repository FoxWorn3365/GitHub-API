...
class Search extends Client {
  public function code(string $query) : object {
    return json_decode(Http::getNoCache("{$this->endpoint}/search/code?q={$query}", $this->token));
  }

  public function commits(string $query) : object {
    return json_decode(Http::getNoCache("{$this->endpoint}/search/commits?q={$query}", $this->token));
  }

  public function issues(string $query) : object {
    $data = json_decode(Http::getNoCache("{$this->endpoint}/search/issues?q={$query}", $this->token));
    $data->type='issue';
  ...
  public function labels(string $query) : object {
    $data = json_decode(Http::getNoCache("{$this->endpoint}/search/labels?q={$query}", $this->token));
    $data->type='label';
    return $data;
  }

  public function repo(string $query) : object {
    $data = json_decode(Http::getNoCache("{$this->endpoint}/search/repositories?q={$query}", $this->token));
    $data->resolve = self::resolve($data);
    $data->type='repo';
    return $data;
  }
  ...
  public function topics(string $query) : object {
    $data = json_decode(Http::getNoCache("{$this->endpoint}/search/topics?q={$query}", $this->token));
    $data->type='topic';
    return $data;
  }

  public function users(string $query) : object {
    $data = json_decode(Http::getNoCache("{$this->endpoint}/search/users?q={$query}", $this->token));
    $data->resolve = self::resolve($data);
    $data->type='user';
    return $data;
  }
